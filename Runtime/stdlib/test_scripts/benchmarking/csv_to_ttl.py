...
import pandas as pd
from rdflib import DC, RDF, XSD, Graph, Literal, Namespace
...
def build_benchmark_graph(dataset_name, generate_small_ttl=False):
    ...
    for idx, row in df.iterrows():
        ...
        g.add(
            (SML["Benchmark{}{}".format(idx, row["model"])], RDF.type, SML["Benchmark"])
        )
        g.add(
            (
                SML["Benchmark{}{}".format(idx, row["model"])],
                SML["targetDataSet"],
                SML["{0}".format(dataset_name)],
            )
        )
        g.add(
            (
                SML["Benchmark{}{}".format(idx, row["model"])],
                MEXALGO["hasAlgorithmClass"],
                MEXALGO[row["model"]],
            )
        )
        ...
        for param_idx, param_info in enumerate(parameters.items()):
            ...
            g.add(
                (
                    ...
                    SML["hasParameter"],
                    SML[
                        "Benchmark{}{}Parameter{}".format(idx, row["model"], param_idx)
                    ],
                )
            )
        ...
        benchmark_type = (
            "Classification" if "Classifier" in row["model"] else "Regression"
        )
...
def build_benchmark_graphs(dataset_names, generate_small_ttl=False):
    for dataset_name in dataset_names:
        try:
            ...
            print("#" * 10)
            print(
                "building benchmarking KG for dataset {0} "
...
if __name__ == "__main__":
    ...
    build_benchmark_graphs(
        dataset_names=DATESET_NAMES, generate_small_ttl=IS_USE_SMALL_SUBTABLE
    )
